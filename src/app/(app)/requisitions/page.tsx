  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center text-slate-500 border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] text-xs">
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Chantier : <strong className="text-slate-800">{req.project?.title}</strong> ({req.project?.code}) • Émis le {formatDate(req.created_at)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canBuy && (req.status === "approved" || req.status === "site_manager_approved") && !req.po_number && (
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold flex items-center gap-1.5 shadow-xs transition"
                    >
                  ...
                  {currentUser?.role === "admin" && (
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-[#8E2424] hover:text-rose-800 border border-rose-200 transition"
                    >
              ...
              {req.supervisor_comment && (
                <div className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <span className="font-semibold text-slate-800">Commentaire superviseur : </span>
                  {req.supervisor_comment}
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-100 max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreatePO} className="space-y-4 text-xs">
              <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200/80 text-amber-900">
                <strong className="block mb-1 font-bold text-amber-950">Règle d&apos;Achat SIX SIGMA :</strong>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800">Devis Fournisseur 1</span>
                  <label className="flex items-center gap-1.5 cursor-pointer text-slate-700">
                    ...
                    />
                    <span className="text-[11px] font-semibold text-emerald-700">Sélectionner comme offre retenue</span>
                  </label>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-800 text-xs focus:outline-none focus:ring-1 focus:ring-slate-300"
                  />
                  <input
                    type="number"
                    step="0.01"
                    ...
                    className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-800 font-mono font-bold text-xs focus:outline-none focus:ring-1 focus:ring-slate-300"
                  />
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800">Devis Fournisseur 2 (Comparatif)</span>
                  <label className="flex items-center gap-1.5 cursor-pointer text-slate-700">
                    <input
                      type="radio"
                      name="selectedSupplier"
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={poSaving}
                  className="px-5 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold transition disabled:opacity-50 shadow-xs"
                >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-100 max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            ...
            <form onSubmit={handleCreateRequisition} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 font-semibold mb-1">Chantier Demandeur *</label>
                  <select
                    value={selectedProjectId}
                    onChange={(e) => setSelectedProjectId(e.target.value)}
                    required
                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-xs focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-300"
                  >
                    {projects.map((p) => (
                ...
                <div className="flex items-center pt-6">
                  <label className="flex items-center gap-2 text-slate-700 font-semibold cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded text-[#8E2424] border-slate-300 focus:ring-[#8E2424]"
                    />
                    <span className="text-[#8E2424] font-bold">Marquer comme URGENT (Priorité Chantier)</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {items.map((it, idx) => (
                    <div
                      key={idx}
                      className="grid grid-cols-12 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 items-center"
                    >
                      <div className="col-span-12 sm:col-span-4">
                        <input
                          ...
                          className="w-full p-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-xs focus:outline-none focus:ring-1 focus:ring-slate-300"
                        />
                      </div>
                      ...
                      <div className="col-span-2 sm:col-span-1 text-right">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-slate-200/60"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Commentaire ou Note Spécifique</label>
                <textarea
                  rows={2}
